# Test Report — Import/Export + Accounting XML Integration

Scope honesty upfront: the original brief asked for **26 automated tests**
including a mock accounting server and a full test suite in the project's
...
- **Commit:** `8104228` (feat(settings): Import & Export hub + Accounting Sync scaffold)
...
| 12 | Accounting  | `GET /api/tally/export/masters` → 200, 1.96 MB well-formed XML           | ✓      |
| 13 | Accounting  | Masters: 5 Groups, 2 Units, 4005 StockItems, 8 Ledgers                   | ✓      |
| 14 | Accounting  | `POST /api/tally/test-connection` no accounting server → clean ODBC-hint error | ✓ |
| 15 | Voucher     | `GET /api/tally/export/vouchers` — 15 VOUCHER blocks in well-formed XML  | ✓      |
| 16 | Voucher     | Re-upload same voucher XML → 0 imported, 15 skipped as duplicates        | ✓      |
| 17 | Voucher     | Hand-crafted XML (1 Sales + 1 Receipt) → 2 imported, 0 errors; total=590 matches CGST+SGST+subTotal | ✓ |
| 18 | Voucher     | Missing product in inventory line → voucher skipped, error explicit      | ✓      |
| 19 | Voucher     | Unsupported VCHTYPE (Journal) → reported in errors, not silently dropped | ✓      |
| 20 | UI          | Import & Export + Accounting Sync pages render, menu items present       | ✓      |
...
| 28 | Accounting XML | `tally-masters-envelope.xml` end-to-end → DB rows                  | Fixture + mock server not written |
| 29 | Accounting XML | `tally-with-quirks.xml` (empty tags, Unicode, `.LIST` nesting)     | Same |
| 30 | Accounting XML | `tally-malformed.xml` fails cleanly                                | Error path on wrong file type works; malformed-XML path not explicitly exercised |
| 31 | Accounting live| Mock HTTP server — push, pull, partial failure, retry, conflict    | Mock server not built |
...
- **Products count in Accounting export** — 4005 stock items is from accumulated
...
2. **Mock accounting server** — `/test-tools/mock-tally-server.js`
   listening on port 9001. Enough of the accounting XML dialect to unit-test
...
Milestones M1 (Excel hub + templates), M2 (Accounting scaffold + masters export),
M3 (Excel Sales/Purchase/Receipts ingestion), and M4 (Accounting voucher
ingestion) are all shipped and functional. The remaining 8 deferred tests
are fixture-generation + mock-accounting infra, not missing features —
