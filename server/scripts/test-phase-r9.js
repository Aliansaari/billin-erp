...
async function preClean() {
  ...
  // path, which creates ledger_entries tied to the test party AND
  // counter-leg entries (Cash) where party_id is NULL. The party-side
  // entry is caught by the party_id filter; the cash-side entry needs
  // the reference_number filter too — without it, ₹X stays in active
  // debits but the matching credit leg gets deleted, producing a
  // ledger imbalance that surfaces on the Integrity screen.
  await sequelize.query(`
    DELETE FROM ledger_entries
     WHERE reference_number LIKE '${PFX}%'
        OR narration LIKE '%${PFX}%'
