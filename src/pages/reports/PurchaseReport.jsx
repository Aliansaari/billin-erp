...
export default function PurchaseReport() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await reportAPI.getPurchaseReport(filters);
  ...
  const handleExport = async () => {
    try {
      const res = await dataAPI.exportExcel('purchases');
      const url = window.URL.createObjectURL(new Blob([res.data]));
      const a = document.createElement('a');
      a.href = url;
      a.download = `purchase_report_${dayjs().format('YYYY-MM-DD')}.xlsx`;
      a.click();
    } catch (e) {
