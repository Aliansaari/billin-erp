...
function ViewModal({ bill, onClose }) {
  ...
  return (
    <Modal open onCancel={onClose} width={960} footer={null}
      title={<span style={{ fontWeight: 700, color: '#B91C1C' }}>Sales Return — {bill.return_number}</span>}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <div style={{ width: 300 }}>
          ...
          <SummaryRow label="Credit Total" value={fmt(bill.total_amount)} bold borderTop color="#B91C1C" />
...
export default function SalesReturnList() {
  ...
  const columns = [
    ...
    { title: 'Customer', dataIndex: ['customer', 'party_name'], width: 180, render: (v) => v || '—' },
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div className="erp-page-header" style={{ padding: '12px 20px', marginBottom: 0, background: '#fff', borderBottom: '1px solid #f0f0f0', flexShrink: 0 }}>
        <div className="erp-page-header-title">
          <Title level={3} style={{ margin: 0, fontWeight: 700, color: '#1f2937' }}>Sales Returns</Title>
          <span style={{ fontSize: 13, color: '#6b7280' }}>{total} returns total</span>
        </div>
        <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/sales-return/new')}
      ...
      <Card bodyStyle={{ padding: 0, display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          <Table columns={columns} dataSource={bills} rowKey="sales_return_id" loading={loading}
