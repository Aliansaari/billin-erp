...
export default function SalesByItem() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    reportAPI.productSalesItems({ from_date: fromDate, to_date: toDate })
      .then((res) => {
        if (cancelled) return;
  ...
  async function generatePdf() {
    ...
    return { blob: doc.output('blob'), fileName: `sales-by-item-${fromDate}_${toDate}.pdf` };
  }
  ...
  async function handleSharePdf() {
    ...
    try {
      const result = await generatePdf();
      const ok = await shareViaNative(result.blob, result.fileName, 'Sales by Item');
  ...
  return (
    <div className="rl-screen drill-in">
      ...
      <div className="rl-top">
        ...
        <h1 className="rl-title">Sales <em>by item</em></h1>
        <button
      ...
      {!loading && filtered.length > 0 && (
        <div className="rl-sticky-footer">
          <span className="rl-footer-count">
            ...
            {summary?.product_count != null && summary.product_count !== filtered.length
              ? ` of ${summary.product_count}`
              : ''}
          </span>
      ...
      {pdfUrl && (
        <div className="rl-pdf-overlay">
          <div className="rl-pdf-toolbar">
            ...
            <span className="rl-pdf-title">Sales by Item</span>
            <button className="rl-pdf-share" onClick={async () => {
              try {
                const resp = await fetch(pdfUrlRef.current);
                const blob = await resp.blob();
                await shareViaNative(blob, `sales-by-item-${fromDate}_${toDate}.pdf`, 'Sales by Item');
