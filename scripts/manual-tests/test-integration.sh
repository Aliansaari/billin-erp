...
LOG=/tmp/billing-erp-integration.log
> "$LOG"
...
COST_LAYERS=$(db billing_erp "SELECT to_regclass('public.cost_layers')::text")
if [ "$COST_LAYERS" = "cost_layers" ]; then ok "cost_layers table exists"
else no "cost_layers table missing" "$COST_LAYERS"; fi

SLLC=$(db billing_erp "SELECT to_regclass('public.sale_line_layer_consumptions')::text")
...
COG=$(db billing_erp "SELECT cogs_method FROM system_settings WHERE setting_id=1")
if [ "$COG" = "weighted_avg" ] || [ "$COG" = "fifo" ]; then ok "system_settings.cogs_method = '$COG'"
else no "cogs_method column missing or empty" "$COG"; fi

STI=$(db billing_erp "SELECT 1 FROM information_schema.columns WHERE table_name='stock_transfer_items' AND column_name='cost_layers_consumed'")
...
ROL=$(db billing_erp "SELECT 1 FROM information_schema.columns WHERE table_name='stock_ledger' AND column_name='is_reversal_of_ledger_id'")
...
IDX=$(db billing_erp "SELECT indexname FROM pg_indexes WHERE indexname='idx_stock_ledger_reversal_of'")
...
IDX_CHQ=$(db billing_erp "SELECT indexname FROM pg_indexes WHERE indexname='cheques_bank_number_dir_active_uniq'")
if [ -n "$IDX_CHQ" ]; then ok "cheques_bank_number_dir_active_uniq exists"
else no "cheque partial unique missing" "$IDX_CHQ"; fi
IDX_PRIM=$(db billing_erp_master "SELECT indexname FROM pg_indexes WHERE indexname='idx_companies_one_primary'")
...
INS=$(db billing_erp "INSERT INTO cost_layers (product_id, godown_id, qty_original, qty_remaining, rate, source_type, acquired_at) SELECT product_id, godown_id, 0.001, 0.001, 0, 'Adjustment', NOW() FROM product_godown_stock LIMIT 1 RETURNING layer_id" 2>&1)
case "$INS" in
  *[0-9]*) ok "cost_layers accepts inserts (layer_id=$INS)"; LAYER_ID=$(echo "$INS" | tr -d '[:space:]'); db billing_erp "DELETE FROM cost_layers WHERE layer_id=$LAYER_ID" >/dev/null ;;
